...
class PathGeneratorService:
    ...
    async def generate_paths(self, job_input: str, current_state: str) -> DualPath:
        ...
        2. Match skills to graph, augment if needed
        3. Assess user state
        ...
        # Step 2: Match to graph
        match_result = await self.job_interpreter.match_to_graph(interpretation)
        matched = match_result["matched"]
        unmatched = match_result["unmatched"]

        # Step 2b: Augment graph with missing skills
        if unmatched:
            logger.info(f"Augmenting graph with {len(unmatched)} new skills")
            await self._augment_graph(unmatched)
            # Re-match after augmentation
            match_result = await self.job_interpreter.match_to_graph(interpretation)
            matched = match_result["matched"]
        ...
        # Step 3: Assess user state
        available_names = [m["graph_node"].name for m in matched]
        assessment = await self.user_state.assess(current_state, interpretation, available_names)
        ...
        # Step 6: Generate explanations for both paths (deduplicate shared nodes)
        all_node_ids = set()
        ...
        explanations = await self._generate_explanations(
            list(all_node_ids), subgraph, interpretation.title
        )

        # Apply explanations to both paths
    ...
    def _topological_sort(self, node_ids: set[str], edges: list) -> list[str]:
    ...
    async def _generate_explanations(
        ...
        """Generate AI explanations for path skills in batches."""
        node_map = {n.id: n for n in subgraph.nodes}
        explanations = {}
        ...
        for i in range(0, len(skill_ids), batch_size):
            ...
            batch_names = [node_map[sid].name for sid in batch_ids if sid in node_map]
            ...
            try:
                ...
                logger.warning(f"Explanation generation failed for batch {i}")
                for sid in batch_ids:
                    explanations[sid] = {
                        "why_needed": "설명을 생성할 수 없습니다.",
                        "job_relevance": "",
                        "connection_to_next": "",
                    }
